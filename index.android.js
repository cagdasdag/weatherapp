...
class havadurumu extends Component {
  ...
  renderData(weather,city) {
    ...
    //ilk gün için
    var today = new Date();
    listofday.push({name: weekday[today.getDay()],temp:(weather.currently.temperature-32)/1.8,icon:(weather.currently.icon).replace(/-/g, '')});
    ...
    for(var i = 1 ; i<=5;i++)
    {
      ...
      listofday.push({name: weekday[today.getDay()],temp:(weather.daily.data[i+1].temperatureMin-32)/1.8,icon:(weather.daily.data[i+1].icon).replace(/-/g, '')});
    }
      ...
      <View style={styles.container}>
        ...
        <View style={styles.front}>
          ...
          <View style={styles.details}>
            <Image source={{uri: listofday[0].icon}} style={styles.icons} />
